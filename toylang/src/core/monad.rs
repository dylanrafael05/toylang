    ...
    where Self: Sized
{
    ...
    fn map_ref_deep<T, F: FnOnce(&<Self::Value as Wrapper>::Value) -> T>(&self, func: F) -> Self::Of<<Self::Value as Wrapper>::Of<T>>
        where Self::Value : Wrapper
    {
        self.map_ref(|x| x.map_ref(func))
    }
    fn map_deep<T, F: FnOnce(<Self::Value as Wrapper>::Value) -> T>(self, func: F) -> Self::Of<<Self::Value as Wrapper>::Of<T>>
        where Self::Value : Wrapper
    {
